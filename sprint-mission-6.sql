...
select *
from orders
order by date desc,time desc limit 10 offset (5 - 1) * 10;
...
select pizza_id, SUM(order_details.quantity * pizzas.price) AS total_revenue
...
sum(order_details.quantity * pizzas.price) as total_amount
from pizza_types
left join pizzas on pizza_types.id = pizzas.type_id
left join order_details on order_details.pizza_id = pizzas.id
